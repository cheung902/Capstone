	...
	for ori_block_num in range(0, ori_max_block):
		...
		for comp_block_num in range(ori_block_num, comp_max_block):
			...
			if block_similarity == 1 and ori_block_num == comp_block_num:
				...
				print("ori block number: ", ori_block_num, "comp block number: ", comp_block_num)
				ori_diff, comp_diff, insertion_num, deletion_num = diff_match(ori_block_text, comp_block_text,
				...
				if (comp_diff != None):
					...
					comp_word_and_line_num = get_diff_word_and_line_num(data_frame= comp_data_frame, diff_list = comp_diff,
																		block_num= comp_block_num, insert_label = insert_label,
																		delete_label = delete_label)

					ori_word_and_line_num = get_diff_word_and_line_num(data_frame=ori_data_frame, diff_list=ori_diff,
																		block_num=ori_block_num, insert_label=insert_label,
																	    delete_label=delete_label)

					comp_word_position = get_word_position(data_frame = comp_data_frame, block_num = comp_block_num,
														   word_and_line_num_list = comp_word_and_line_num,
														   position_list = comp_word_position)


					ori_word_position = get_word_position(data_frame=ori_data_frame, block_num=ori_block_num,
...
def get_diff_word_and_line_num(data_frame, diff_list, block_num, insert_label, delete_label):
...
def adjustWordNum(data_frame):
	for i in range(len(data_frame)):
		if data_frame.loc[i, 'line_num'] > 1 and data_frame.loc[i, 'word_num'] !=0:
