...
# testing parameters
pdfFile = "EasyOCR/App_form_2.pdf"
txtFile = "EasyOCR/App_form_2.txt"
csvFile = "EasyOCR/Contract.csv"
page = 13
ocr_time = 62.887774723

def get_ocr_data(results, ocr_time):
    print("-----------------------------------")
    print("Getting OCR data")

    column_name = ["coor", "text", "conf"]
    ocr_data = results

    df_ocr = pd.DataFrame(ocr_data, columns=column_name)
...
def get_csv(txtFile, results, ocr_time):
    ...
    pdflist, ocr_time, conf_list = get_ocr_data(results, ocr_time)
...
def throughput(csvFile, page):
    ...
    df_original = pd.read_csv(csvFile)
...
def error_df(csvFile):
    ...
    error_df["conf"] = round(error_df["conf"]*100,1)
...
# get_csv(txtFile, results, ocr_time)
