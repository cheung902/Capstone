...
def get_position(data_frame, diff_list, insert_label, delete_label):
	...
	for num, element in enumerate(diff_list_split):
		if insert_label_start in element:
			...
			append_position_list(data_frame = data_frame, list = position_list, num = num, word = word, insert_or_delete="1")
			num +=1
...
def addTextLabel(text, label):
	...
	if lenWord_space > lenWord or lenWord >1:
...
def append_position_list(data_frame,list, num, word, insert_or_delete):

	word_num = num + 1
	word_position = data_frame[(data_frame['text'] == word)
							   & (data_frame['word_num'] == word_num)]
	list.append([word_position.iloc[0]['page_num'],
